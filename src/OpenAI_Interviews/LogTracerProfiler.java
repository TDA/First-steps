...
class FunctionFrame {
    ...
    public FunctionFrame(String functionName, String event, Integer timestamp, Integer depth) {
...
public class LogTracerProfiler {
    public String trace(List<Function> functionCalls) {
        ...
        var depth = 0;
        Map<Integer, Integer> timeSpentInDepth = new HashMap<>();
        for (Function f : functionCalls) {
            StringBuilder subStringBuilder = new StringBuilder();
            if ("enter".equals(f.event())) {
                // push on to stack
                callStack.push(new FunctionFrame(f.functionName(), f.event(), f.timestamp(), ++depth));
                ...
                var selfTime =  totalTime - timeSpentInDepth.getOrDefault(depth + 1, 0);
                timeSpentInDepth.put(depth, timeSpentInDepth.getOrDefault(depth, 0) + totalTime);
